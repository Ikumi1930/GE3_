...
#pragma region 描画初期化処理
  ...
  // 上（下面とY座標の符号
	...
	// インデックスデータ
	uint16_t indices[] = {0,  1,  3,  3,  2,  0,  4,  5,  7,  7,  6,  4,  8,  9,  11, 11, 10, 8,
...
#pragma endregion
	...
	XMMATRIX matProjection = XMMatrixPerspectiveFovLH(
	    XMConvertToRadians(45.0f), (float)WinApp::window_width / WinApp::window_height, 0.1f,
	    1000.0f);
	...
	while (true) {
		...
		// 座標操作
		if (input_->PushKey(DIK_UP) || input_->PushKey(DIK_DOWN) || input_->PushKey(DIK_RIGHT) ||
		...
		srvGpuHandle.ptr += textureIndex * device->GetDescriptorHandleIncrementSize(
