...
void TextureManager::LoadTexture(const std::wstring& filePath) 
{
	...
	result = DirectX::GenerateMipMaps(image.GetImages(), image.GetImageCount(), image.GetMetadata(),
	...
	dxCommon_->GetDevice()->CreateShaderResourceView(data.resource.Get(),
	                                             &srvDesc, data.sevHandleCPU);
}
