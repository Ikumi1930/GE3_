...
void DirectXCommon::DepthBufferInitialize() {
	...
	result = device->CreateCommittedResource(
	    &depthHeapProp, D3D12_HEAP_FLAG_NONE, &depthResourceDesc,
...
ID3D12DescriptorHeap* DirectXCommon::CreateDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE heapType, UINT numDescripots, bool shaderVisible) {
	...
	descriptorDesc.Type = heapType;
	descriptorDesc.NumDescriptors = numDescripots;
	descriptorDesc.Flags = shaderVisible ? D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE : D3D12_DESCRIPTOR_HEAP_FLAG_NONE;

	HRESULT result = device->CreateDescriptorHeap(&descriptorDesc, IID_PPV_ARGS(&descriptorHeap));
	assert(SUCCEEDED(result));
